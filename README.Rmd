...
It is important to consider that applying the log operation on the average salary will change the interpretation of the results for the final regression, which will be explained on the next section.
...
plot = ggplot(data = dataset, aes(dataset$employment_time, log(dataset$norm_avg_salary), 
                                  color = dataset$CBO2002)) + geom_point() + 
  geom_smooth(color = "black") + labs(x = "employment time", 
...
The Mincer earnings function was published in 1974 and it models the salary in function of year of education (scholarity), experience in the market (employment time and age) and other factors (such as gender). It is one of the most widely used models in empirical economics.

The Mincer model is defined by the function that follows:

ln norm_avg_salary = B0 + B1 scholarity + B2 X + B3 X^2 + G' gender + error

Where X is experience in the market, B0 is the salary of an individual with no education and no experience, B1, B2, B3 and G' are the adjust parameters.
...
* Sites aboutt Mincer Earnings Function:
