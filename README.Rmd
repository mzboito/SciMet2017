...
For these plots, we are using the log of the average salary. This change does not affect the values order, it only escales the data, helping to visualize it.

# age x salary
```{r}
qplot(log(economist$avg_salary), economist$age, xlab = "Average Salary", ylab = "Age") 
```
...
```{r}
qplot( (log(economist$avg_salary) / economist$contract_hours), economist$age, xlab = "Average Salary per Contract hours", ylab = "Age") 

```

# gender x salary
```{r}
ggplot(data=economist, aes(x= factor(economist$gender), y=economist$avg_salary)) + geom_boxplot()
```


# scholarity x salary
```{r}
ggplot(data=economist, aes(x= factor(economist$Scholarity), y=economist$avg_salary)) + geom_boxplot()
```
